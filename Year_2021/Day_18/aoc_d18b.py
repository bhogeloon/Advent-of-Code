...
Part 2:
- Perform the addition on all possible combinations and keep track of the largest
  value.
"""
...
def get_largest_magnitude(lines):
    '''Main function'''
    snail_fishes = read_snail_fish(lines)
    largest_magnitude = 0

    for i in range(len(snail_fishes)):
        for j in range(len(snail_fishes)):
            # If both are the same, skip
            if i == j:
                continue

            new_fish = add_snail_fish(snail_fishes[i], snail_fishes[j])
            magnitude = calculate_magnitude(new_fish)
...
if __name__ == '__main__':
    ...
    print("Largest Magnitude:", get_largest_magnitude(lines))
